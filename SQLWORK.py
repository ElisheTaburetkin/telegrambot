...
class DataBase:
    ...
    userid TEXT, userfromid TEXT);""")
        ...
        name TEXT, description TEXT,
        photo TEXT, price REAL,
        userid TEXT, userfromid TEXT, date TEXT);""")
    ...
    async def get_ad(self, type):
        if type!='Все категории':
            ads = self.cur.execute(f""" SELECT * FROM AD_MAIN WHERE type='{type}' ORDER BY id DESC;""").fetchall()
            counter = 1
            page = 1
            ADS = []
            ad = []
            ...
            ads = self.cur.execute(f""" SELECT * FROM AD_MAIN ORDER BY id DESC;""").fetchall()
            counter = 1
            page = 1
            ADS=[]
            ad=[]
    ...
    async def get_my_ads(self, userid):
        ads = self.cur.execute(f""" SELECT * FROM AD_MAIN WHERE userfromid = '{userid}' ORDER BY id DESC;""").fetchall()
        counter = 1
        page = 1
        ADS = []
        ad = []
    ...
    async def accept_ad(self, id):
        try:
            ad = self.cur.execute(f""" SELECT * FROM AD WHERE id='{id}';""").fetchone()
            self.cur.execute(f"""DELETE FROM AD WHERE id='{id}';""")
            self.cur.execute(f"""INSERT INTO AD_MAIN(type, name, description, photo, price, userid, userfromid, date)
                        VALUES('{ad[1]}', '{ad[2]}', '{ad[3]}', '{ad[4]}','{ad[5]}','{ad[6]}','{ad[7]}','{date.today()}');""")
            self.conn.commit()
    ...
    async def get_stats(self):
        ...
        labels = ['Недвижимость Москва и МО','Недвижимость Санкт-Петербург и ЛО','Недвижимость Краснодарский край','Недвижимость другие регионы РФ','Зарубежная недвижимость','Коммерческая недвижимость','Аренда Москва','Аренда регионы РФ','Продажа бизнеса','Продажа оборудования','Другое']
        values = []
        labelsn = []
        for i in labels:
    ...
    async def get_len_ads(self):
        return len(self.cur.execute(f""" SELECT * FROM AD_MAIN;""").fetchall())

    async def get_len_users(self):
